...
const ProductTabs = ({currentProduct}) => {
  ...
  const handleSubmitReview = async (e) => {
    ...
    try {
      const response = await addReview(currentProduct._id, data.rating, data.comment);
      fetchReviews();
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="overflow-y-auto max-h-[400px] no-scrollbar">
              <h2 className="text-xl font-semibold">
                {currentProduct?.reviewCount} {currentProduct?.reviewCount === 1 ? "Review" : "Reviews"} for "{currentProduct?.name}"
              </h2>
              { reviews.length > 0 && reviews.map((review) => (<div className="mt-4 border-b pb-4">
              <div className="flex gap-4">
                <div
                  className="w-14 h-14 rounded-full flex items-center justify-center text-2xl text-white bg-yellow-400 hover:ring-4 hover:ring-yellow-400 transition-all duration-300"
                >{review.user.username.split('')[0].toUpperCase()}</div>
                <div>
                <h3 className="font-semibold">{review.user.username}</h3>
                  <p className="text-sm text-gray-500">{review.createdAt.split('T')[0]}</p>
                  {/* Rating */}
            ...
            <div>
              ...
              <div className="mt-4">
                ...
                <div className="flex mt-2 gap-2">
                  {[1, 2, 3, 4, 5].map(value => (
                    <div 
                    className={`w-6 h-6 flex items-center justify-center text-lg text-white bg-yellow-400 hover:ring-4 hover:ring-yellow-400 ${data.rating === value ? 'ring-4 ring-yellow-500 bg-yellow-500' : ''} hover:cursor-pointer transition-all duration-300`} 
                    key={value}
                    value={data.rating}
                    onClick={() => {
                     if(data.rating !== value) setData({...data, ["rating"]: value})
              ...
              <div className="mt-4">
                <label htmlFor='review' className="block text-sm font-medium cursor-pointer">Your Review <span className="text-red-500">*</span></label>
                <textarea
              ...
              <button onClick={handleSubmitReview} className="mt-4 bg-yellow-500 text-white py-2 px-4 hover:bg-yellow-600">
                Leave Your Review
              </button>
            </div>
          </div>

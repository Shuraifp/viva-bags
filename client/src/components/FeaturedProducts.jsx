...
const FeaturedProducts = () => {
    ...
    const fetchFeaturedProducts = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_URL}/user/products/featured`
        );
  ...
  return (
    <section className="pb-12">
      <h2 className="text-3xl mt-28 font-bold mb-8 ml-6 text-center md:flex md:items-center md:justify-start">
        FEATURED PRODUCTS
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
          {products?.map((product) => (
            <Link key={product._id} to={`/product/${product._id}`}>
              <div key={product._id} className="border p-4 shadow-lg bg-white">
                <img
                  src={
                  ...
                  alt={product.name}
                  className="w-full h-48 object-cover mb-4 hover:scale-110 transition duration-300"
                />
                ...
                <p className="text-xl text-center text-yellow-500 font-medium mt-2">
                  ...
                  <span className=" text-base font-normal text-gray-400 ml-3 line-through">
                    {product?.discountedPrice &&
                    product?.discountedPrice > 0 &&
                    product?.discountedPrice < product?.regularPrice
                      ? product?.regularPrice

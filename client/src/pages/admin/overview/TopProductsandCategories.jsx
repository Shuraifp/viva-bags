  ...
  return (
    <div className="flex justify-around p-6 gap-4">
        ...
        <div className='flex flex-col mt-16'>
          <div className='flex w-full justify-center'>
            <div className='m-2 flex flex-col gap-0.5 justify-end'>
              ...
              <div className='text-xl bg-yellow-100 font-semibold text-blue-400 mt-1 px-2 py-1 text-center'>
                <p> {products[0]?.name}</p>
            ...
            <div className='m-2 flex flex-col gap-0.5 justify-end'>
              <h2 className="text-3xl font-bold mb-4 px-4 text-gray-500 text-center">Top Selling Products</h2>
              <img className='h-[280px]' src={import.meta.env.VITE_API_URL + products[1]?.images[0].url} alt="" />
              <div className='text-lg font-semibold text-blue-400 bg-yellow-100 mt-1  px-2 py-1 text-center'>
                <p> {products[1]?.name}</p>
          ...
          <div className='flex justify-center'>
            <div className='flex'>
              <div className='m-2 h-fit'>
                ...
                <div className='text-md font-semibold text-blue-400 bg-yellow-100 mt-1 px-2 py-1 text-center'>
                  <p> {products[2]?.name}</p>
      ...
      <div className='m-1/3 mt-20 flex flex-col gap-28'>
        <div>
        <h2 className="text-2xl text-gray-500 font-bold mb-4 text-center">Top Selling Categories</h2>
        {categories.map((category, index) => (
          <div key={index} className='flex items-center justify-between p-2 border-b-2 border-gray-200'>
            ...
            <p className='text-lg text-green-500 font-semibold'>{category.total}</p>
          </div>
        ))}
        </div>
        
        <div>
        <h2 className="text-2xl text-gray-500 font-bold mb-4 text-center">Top Selling Brands</h2>
        {brands.map((brand, index) => (
          <div key={index} className='flex items-center justify-between p-2 border-b-2 border-gray-200'>
            <p className='text-lg font-semibold'>{brand.brand}</p>
            <p className='text-lg text-green-500 font-semibold'>{brand.total}</p>
          </div>
        ))}
      </div>
      </div>

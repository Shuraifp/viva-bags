...
import { addCoupon, getCoupons, updateCoupon, deleteOrRestoreCoupon } from "../../api/coupon";
...
const CouponManagement = () => {
  ...
  useEffect(() => {
    const fetchCoupons = async () => {
      try {
        const response = await getCoupons(currentPage, limitPerPage, filter, sort,search);
    ...
    try {
      if(isEditing) {
        ...
        if (response.status === 200) {
          ...
          setCoupons(coupons.map((item) => item._id === couponData._id ? response.data.coupon : item));
          setCouponData({
            code: "",
            discountType: "",
            discountValue: "",
  ...
  const handleDeleteAndRestore = async (id) => {
    try {
      ...
      if (response.status === 200) {
        toast.success(response.data.message);
        setCoupons(coupons.map((item) => item._id === id ? response.data.coupon : item));
      } else {
        toast.error('Something unexpected occured!');
      }
    } catch (err) {
        ...
        <tbody>
          {coupons?.map((item, index) => (
            ...
            >
              ...
              <td className="py-2 px-4">{item.discountType === "percentage" ? `${item.discountValue}%` : `₹${item.discountValue}`}</td>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-4">
            ...
            {errors.minimumPurchase && (
              <span className="text-red-500 text-sm">{errors.minimumPurchase}</span>
            )}
          </div>

          {/* Maximum Discount */}
         { couponData.discountType === 'percentage' && <div className="mb-4">

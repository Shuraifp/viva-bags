    ...
    <form onSubmit={handleSubmit}>
      <h3 className="text-xl font-bold my-4">Choose Color</h3>
        <div className="10px">
          ...
          />
          <button type="button" className="ml-5 text-lg bg-red-50 px-3 py-1 rounded-md text-red-500" onClick={() => removeColor()}>
            Remove
          </button>
          {errors.colorhex && <span className="text-red-500 text-sm">{errors.colorhex}</span>}
          {errors.colorname && <span className="text-red-500 text-sm">{errors.colorname}</span>}
        </div>
    </form>
  );
};

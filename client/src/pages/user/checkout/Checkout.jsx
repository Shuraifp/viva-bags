...
const CheckoutPage = () => {
      ...
      try {
        ...
        if (err.response) {
          if (
            ...
            err.response.data.message === "User is blocked"
          ) {
            logout();
          }
        }
  ...
  const handlePlaceOrder = async () => {
    if (paymentMethod === "Razorpay") {
      ...
      if (total < 1000) {
        toast.error(
          "Minimum order amount for COD is ₹1000. Please select a different payment method."
        );
        return;
      }
      placeOrder();
      ...
      const orderData = {
        products: cartItems.map((item) => ({
          ...
          discount: item.product.discountedPrice
            ? item.product.regularPrice - item.product.discountedPrice
            : 0,
  ...
  const calculateTotalWithCoupon = () => {
    if (coupon.discountType === "percentage") {
      return (subtotal * (1 - coupon.discountValue / 100) + shipping).toFixed(
        2
      );
    } else {
  ...
  const discount = coupon
    ? coupon.discountType === "percentage"
      ...
      : `₹${coupon.discountValue}`
    : 0;
  let total = coupon?.code
    ? calculateTotalWithCoupon()
    : calculateTotalwithoutCoupon();
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mx-8 my-8">
        ...
        <div>
          ...
          <div className="bg-white h-fit p-6 py-8">
            <div>
              <p className="font-semibold text-stone-700 mb-3 text-lg">
                Products
              </p>
              {cartItems?.map((item) => (
          ...
          <div>
            ...
            <div className="bg-white h-fit p-6 pt-6 pb-10 text-lg text-gray-600">
              ...
              <button
                ...
                className="w-full mt-4 py-4 bg-yellow-400 hover:bg-yellow-500 text-lg text-slate-700"
              >
                Place Order
              </button>
            </div>

    ...
    const deleteAdds = async () => {
      try {
        ...
        if (response.status === 200) {
          ...
          setAddresses(
            addresses.filter(
              (address) => address._id !== response.data.deletedAddress._id
            )
          );
        } else {
          toast.error(response.data.message);
    ...
    <div className="bg-slate-100 md:min-h-screen p-6 my-4">
      ...
      <ul className="mt-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {addresses.map((address, i) => (
          <li
            key={address._id}
            className="flex flex-col justify-between relative bg-white shadow-md px-5 py-6 mt-2"
          >
            ...
            <label
              ...
              className="mt-4 text-yellow-600 flex gap-1 text-sm"
            >
              <input type="radio" checked={address.isDefault} />
              Default Address
            </label>
          </li>
        ))}
      ...
      <AddAddress
        addressFormScrollRef={addressFormScrollRef}
        editing={editing}

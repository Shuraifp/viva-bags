...
export const generateSalesReport = async (req, res) => {
  ...
  try {
    ...
    salesData = await Order.aggregate([{ $match: matchStage }]);
...
export const downloadReport = async (req, res) => {
  ...
  try {
    ...
    const totalDiscountValue = parseInt(
      (totalDiscount + productDiscount).toFixed(2)
    );
    ...
    const newAmount = salesData.reduce(
  ...
  try {
    ...
    if (filter === "Yearly") {
      ...
      const daysDifference = Math.floor(
        (nearestThursday - yearStart) / (24 * 60 * 60 * 1000)
      );

...
export const createOrder = async (req, res) => {
  ...
  try {
    ...
    const newOrderNumber = `ORD${new Date().getFullYear()}${
      latestOrder.length > 0
        ? Number(latestOrder[0].orderNumber.split("").splice(7).join("")) + 1
        : 1001
    }`;
    ...
    const validatedCoupon = coupon
      ? {
          ...
          discountValue: !isNaN(Number(coupon.discountValue))
...
export const getAllOrdersForUser = async (req, res) => {
  ...
  try {
    ...
    const totalPages = Math.ceil(
      (await Order.find(filters).countDocuments()) / limitPerPage
    );
...
export const generateInvoice = async (req, res) => {
  ...
  try {
    ...
    doc
      .fontSize(12)
      .fillColor("#000000")
      .text(`Customer: ${order.address.fullName}`)
      .text(`Email: ${order.address.email || "N/A"}`)
      .text(`Mobile: ${order.address.mobile}`)
      .text(
        `Address: ${order.address.address}, ${order.address.locality}, ${order.address.state}, ${order.address.country} - ${order.address.pincode}`
      )
      .moveDown(1);
    ...
    const drawRow = (columns, y) => {
      ...
      columns.forEach((col, index) => {
        doc
          .fillColor(index === 0 ? "#E8F6F3" : "#D5DBDB")
          .rect(x, y, headers[index].width, rowHeight)
          .fill();
        doc
          .fillColor("#000000")
          .fontSize(10)
    ...
    doc
      .fontSize(12)
      .fillColor("#000000")
      .text(`Subtotal: ₹${order.totalAmount}`, tableLeft, y, { align: "left" });
    y += 20;
    if (order.coupon.discountValue > 0) {
      ...
      doc.text(`Coupon Discount: ₹${couponDiscount}`, tableLeft, y, {
        align: "left",
      });
      y += 20;
    }
    doc.text(`Shipping Cost: ₹${order.shippingCost}`, tableLeft, y, {
      align: "left",
    });
    y += 20;
    doc.text(
...
export const cancelOrder = async (req, res) => {
  try {
    ...
    const uncancelledProducts = order.products.filter(
...
export const cancelOrderItem = async (req, res) => {
  try {
    ...
    const allProductsSameStatus = order.products.every(
      (item) => item.status === product.status
    );
    ...
    if (order.paymentStatus === "Completed") {
      ...
      if (isEligibleForCoupon) {
        ...
        order.totalAmount =
          currentCostOfProducts +
          order.shippingCost -
          (order.coupon.discountType === "percentage"
            ? currentCostOfProducts * (order.coupon.discountValue / 100)
            : order.coupon.discountValue);
      } else {
        refundAmount =
    ...
    res
      .status(200)
      .json({ message: "Item cancelled successfully", order: updatedOrder });
  } catch (error) {
    console.log(error);
...
export const requestItemReturn = async (req, res) => {
  try {
    ...
    const allProducts = order.products.filter(
    ...
    res
      .status(200)
      .json({
        message: "Return request for item submitted",
        order: updatedOrder,
      });
  } catch (error) {
    res
      .status(400)
      .json({ message: "Error requesting item return", error: error.message });
  }
};
...
export const requestOrderReturn = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({
        message: "Return request for order submitted",
        order: updatedOrder,
      });
  } catch (error) {
    res
      .status(400)
      .json({ message: "Error requesting order return", error: error.message });
  }
};
  ...
  try {
    ...
    if (uncancelledProducts.length >= 1) {
      if (uncancelledProducts.some((item) => item.status === "Pending")) {
        ...
        uncancelledProducts.some((item) => item.status === "Shipped") &&
        !uncancelledProducts.some((item) => item.status === "Pending")
      ) {
  ...
  try {
    const orders = await Order.find({
      $or: [
        ...
        {
          products: {
            $elemMatch: { isReturnRequested: true, returnStatus: "Pending" },
          },
        },
      ],
    });
    ...
    res
      .status(400)
      .json({
        message: "Error fetching pending returns",
        error: error.message,
      });
  }
};
...
export const updateReturnStatus = async (req, res) => {
  try {
    ...
    if (product.status === "Cancelled" || product.status === "Returned") {
      return res
        .status(400)
        .json({ message: "Action not allowed on this product" });
    }
    ...
    if (variantIndex !== -1) {
      ...
      return res
        .status(400)
        .json({
          message: `Variant size ${product.size} not found for product`,
        });
    }
    ...
    if (returnStatus === "Completed") {
      ...
      if (order.coupon.discountValue > 0) {
        ...
        if (coupon) {
          isEligibleForCoupon =
            costOfRemainingProducts >= coupon.minimumPurchase;
        }
      }
      if (isEligibleForCoupon) {
        refundAmount = product.price * product.quantity;
        order.totalAmount =
          costOfRemainingProducts +
          order.shippingCost -
          (order.coupon.discountType === "percentage"
            ? costOfRemainingProducts * (order.coupon.discountValue / 100)
            : order.coupon.discountValue);
      } else {
        refundAmount =
          product.price * product.quantity -
    ...
    if (
      allProducts.every(
        (item) =>
          item.returnStatus === returnStatus && returnStatus !== "Completed"
      )
    ) {
      order.isReturnRequested = true;
    ...
    return res
      .status(400)
      .json({ message: "Error updating return status", error: error.message });
  }
};
